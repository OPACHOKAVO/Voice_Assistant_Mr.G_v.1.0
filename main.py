...
class Assistant(QtWidgets.QMainWindow, interface.Ui_MainWindow, threading.Thread):
    ...
    def __init__(self):
        ...
        self.cmds = {
            ...
            ('пока', 'вырубись', 'пауза', 'стоп'): self.quite,
            ...
            'какая загрузка памяти'): self.check_memory,
            ('включи музыку', 'поставь музыку'): self.music,
        ...
        self.ndels = ['Мистер джи', 'Mr.G', 'ладно', 'не мог бы ты', 'пожалуйста',
                      'текущее', 'сейчас']
        ...
        self.commands = [
            ...
            'заметки',
            'загруженость компьютера', 'загруженость системы', 'какая загрузка', 'загруженность памяти', 'какая загрузка памяти'
    ...
    def web_search(self, search):
        ...
        youtube = [
            ...
            'найти видео', 'ищи видео', 'ищи на ютубе', 'ютуб', 'ютюбе', 'найди на ютюбе' 'видео', 'открой видео', 'открой на ютубе', 'открой видео на ютубе']

        words = ['найди', 'найти', 'ищи', 'кто такой', 'что такое']
        remove = ["пожалуйста", "ладно", "давай", "сейчас", 'с именем']
    ...
    def recognizer(self):
        ...
        elif self.text.startswith(('создай файл', 'создать файл', 'сделай файл')):
            self.create_file_on_folder(self.text)

        elif self.text.startswith(('Заведи будильник','поставь будильник', 'поставить будильник', 'запусти будильник', 'запустить будильник', 'установи будильник')):
    ...
    def opener(self, task):
        ...
        def open_discord():
            system(r"C:\Users\Master\AppData\Local\Discord\Update.exe --processStart Discord.exe")
            system("cls")
    ...
    def quite(self):
        self.talk(choice(['Надеюсь мы скоро увидимся', 'Рад был помочь', 'Я отключаюсь']))
        self.engine.stop()
        system('cls')
        sys.exit(0)
    ...
    def hello(self):
        self.talk(choice(['Чем могу помочь?', 'Здраствуйте', 'Приветствую']))
